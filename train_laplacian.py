...
def GraphFiltration(gaussians):
    ...
    neighbors= rlbe.neighborhoodMahalanobis_bilateral(points, norms, RT_inverse, 1e-5, 80)
...
def training(dataset, opt, pipe, testing_iterations, saving_iterations, checkpoint_iterations, checkpoint, debug_from, prune_iterations_cov):
    first_iter = 0
    ...
    for iteration in range(first_iter, opt.iterations + 1):        
        ...
        with torch.no_grad():
            ...
            if (iteration in saving_iterations):
                ...
                laplacian_gaussian(scene.gaussians, 100, tb_writer, iteration) # w/o normals
                laplacian_gaussian2(scene.gaussians, 100, tb_writer, iteration) # with normals
                index = GraphFiltration(scene.gaussians)
                laplacian_gaussian_f_pc(scene.gaussians, index, 100, tb_writer, iteration)
                laplacian_gaussian_f_g(scene.gaussians, index, 100, tb_writer, iteration)
            ...
            # pruning wrt variance
            if iteration in prune_iterations_cov:
                print("Pruning wrt covariance")
                print("Before pruning Gaussian size = ",len(gaussians.get_xyz))
            #     norms, (covs, S) = compute_norm(gaussians)
            #     eig3 = S[..., 2]
            #     eig2 = S[..., 1]
            #     quantile = np.quantile(eig3, 0.9)
            #     mask = eig3.squeeze() >= quantile
                mask = torch.ones(len(gaussians.get_xyz))
                index = GraphFiltration(gaussians)
                mask[index] = 0
                mask = mask > 0.5
                gaussians.prune_points(mask)
                torch.cuda.empty_cache()
                gaussians.compute_3D_filter(cameras=trainCameras)
                print("After pruning Gaussian size = ",len(gaussians.get_xyz))
...
if __name__ == "__main__":
    ...
    parser.add_argument("--prune_iterations_cov", nargs="+", type=int)
    ...
    training(lp.extract(args), op.extract(args), pp.extract(args), args.test_iterations, args.save_iterations, args.checkpoint_iterations, args.start_checkpoint, args.debug_from, args.prune_iterations_cov)
