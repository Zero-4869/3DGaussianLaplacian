...
std::tuple<SparseMatrix<double>, SparseMatrix<double>> buildGaussianLaplacian(const DenseMatrix<double>& vMat, std::vector<Vector3> normals, double mollifyFactor, size_t nNeigh){
  SimplePolygonMesh cloudMesh;
...
PYBIND11_MODULE(robust_laplacian_bindings, m) {
  ...
  m.def("buildGaussianLaplacian", &buildGaussianLaplacian, "build the 3d-Gaussian Laplacian",
      py::arg("vMat"), py::arg("normals"), py::arg("mollifyFactor"), py::arg("nNeigh"));
}
